...
// DkAppManagerDialog --------------------------------------------------------------------
DkAppManagerDialog::DkAppManagerDialog(DkAppManager *manager /* = 0 */,
                                       QWidget *parent /* = 0 */,
...
void DkAppManagerDialog::onAddButtonClicked()
{
    // load system default open dialog
    QString appFilter;
    QString defaultPath;
    ...
    defaultPath = QStandardPaths::writableLocation(
        QStandardPaths::ApplicationsLocation); // still retrieves startmenu on windows
#endif
...
void DkAppManagerDialog::onRunButtonClicked()
{
    accept();
...
void DkSearchDialog::init()
{
    ...
    connect(mSearchBar, &QLineEdit::textChanged, this, &DkSearchDialog::onSearchBarTextChanged);
    ...
    connect(mResultListView, &QListView::clicked, this, &DkSearchDialog::onResultListViewClicked);
    connect(mResultListView, &QListView::doubleClicked, this, &DkSearchDialog::onResultListViewDoubleClicked);

    mFilterButton = new QPushButton(tr("&Filter"), this);
    connect(mFilterButton, &QPushButton::pressed, this, &DkSearchDialog::onFilterButtonPressed);
...
void DkSearchDialog::onSearchBarTextChanged(const QString &text)
{
    DkTimer dt;
    ...
    qDebug() << "searching [" << text << "] - converted to individual keywords [" << text.split(" ")
             << "] takes: " << dt;
    mCurrentSearch = text;
    ...
    if (mResultList.empty()) {
        ...
        mResultListView->selectionModel()->setCurrentIndex(mStringModel->index(0, 0),
...
void DkSearchDialog::onResultListViewDoubleClicked(const QModelIndex &modelIndex)
{
...
void DkSearchDialog::onResultListViewClicked(const QModelIndex &modelIndex)
{
    if (modelIndex.data().toString() == mEndMessage)
        mStringModel->setStringList(makeViewable(mResultList, true));
}
...
void DkSearchDialog::onFilterButtonPressed()
{
...
void DkResizeDialog::createLayout()
{
    ...
    connect(mWPixelSpin,
            QOverload<double>::of(&DkSelectAllDoubleSpinBox::valueChanged),
            this,
    ...
    connect(mHPixelSpin,
            QOverload<double>::of(&DkSelectAllDoubleSpinBox::valueChanged),
            this,
            &DkResizeDialog::onHPixelSpinValueChanged);
    ...
    connect(mSizeBox,
            ...
            this,
            &DkResizeDialog::onSizeBoxCurrentIndexChanged);
    ...
    connect(mHeightSpin,
            QOverload<double>::of(&DkSelectAllDoubleSpinBox::valueChanged),
            this,
            &DkResizeDialog::onHeightSpinValueChanged);
    ...
    connect(mUnitBox,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this,
            &DkResizeDialog::onUnitBoxCurrentIndexChanged);
    ...
    connect(mResolutionSpin,
            QOverload<double>::of(&DkSelectAllDoubleSpinBox::valueChanged),
            this,
            &DkResizeDialog::onResolutionSpinValueChanged);
    ...
    connect(mResUnitBox,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this,
            &DkResizeDialog::onResUnitBoxCurrentIndexChanged);
    ...
    connect(mResampleCheck, &QCheckBox::clicked, this, &DkResizeDialog::onResampleCheckClicked);
    ...
    connect(mResampleBox,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this,
            &DkResizeDialog::onResampleBoxCurrentIndexChanged);
    ...
    connect(mGammaCorrection, &QCheckBox::clicked, this, &DkResizeDialog::onGammaCorrectionClicked);
...
void DkResizeDialog::updatePixelHeight()
{
    ...
    float units = mResFactor.at(mResUnitBox->currentIndex()) * mUnitFactor.at(mUnitBox->currentIndex());
    float pixelHeight = (mSizeBox->currentIndex() != size_percent)
        ? qRound(height * mExifDpi / units)
        : qRound(1000.0f * height * mExifDpi / (units * mImg.height())) / 10.0f;
...
void DkResizeDialog::updatePixelWidth()
{
    ...
    float pixelWidth = (mSizeBox->currentIndex() != size_percent)
        ? qRound(width * mExifDpi / units)
        : qRound(1000.0f * width * mExifDpi / (units * mImg.width())) / 10.0f;
    mWPixelSpin->setValue(pixelWidth);
}

void DkResizeDialog::onLockButtonDimClicked()
{
...
void DkResizeDialog::onLockButtonClicked()
{
...
void DkResizeDialog::onWPixelSpinValueChanged(double val)
{
    ...
    int newHeight = (mSizeBox->currentIndex() != size_percent)
        ? qRound((float)val / (float)mImg.width() * mImg.height())
        : qRound(val);
...
void DkResizeDialog::onHPixelSpinValueChanged(double val)
{
    ...
    int newWidth = (mSizeBox->currentIndex() != size_percent) ? qRound(val / (float)mImg.height() * (float)mImg.width())
                                                              : qRound(val);
...
void DkResizeDialog::onWidthSpinValueChanged(double val)
{
...
void DkResizeDialog::onHeightSpinValueChanged(double val)
{
...
void DkResizeDialog::onResolutionSpinValueChanged(double val)
{
    mExifDpi = (float)val;
...
void DkResizeDialog::onUnitBoxCurrentIndexChanged(int)
{
    updateHeight();
    updateWidth();
    // initBoxes();
...
void DkResizeDialog::onSizeBoxCurrentIndexChanged(int idx)
{
    if (idx == size_pixel) {
        mWPixelSpin->setDecimals(0);
        mHPixelSpin->setDecimals(0);
...
void DkResizeDialog::onResUnitBoxCurrentIndexChanged(int)
{
    updateResolution();
    // initBoxes();
}

void DkResizeDialog::onResampleCheckClicked()
{
...
void DkResizeDialog::onGammaCorrectionClicked()
{
    drawPreview(); // diem: just update
}

void DkResizeDialog::onResampleBoxCurrentIndexChanged(int)
{
    drawPreview();
}
...
QImage DkResizeDialog::resizeImg(QImage img, bool silent)
{
    ...
    if (mSizeBox->currentIndex() == size_percent)
        newSize = QSize(qRound(mWPixelSpin->value() / 100.0f * mImg.width()),
                        qRound(mHPixelSpin->value() / 100.0f * mImg.height()));
    else
    ...
    if (newSize.width() < mWPixelSpin->minimum() || newSize.width() > mWPixelSpin->maximum()
        ...
        if (!silent) {
            ...
            errorDialog.setText(
                tr("Sorry, but the image size %1 x %2 is illegal.").arg(newSize.width()).arg(newSize.height()));
            errorDialog.show();
            errorDialog.exec();
        }
    }
...
bool DkShortcutDelegate::editorEvent(QEvent *event,
...
void DkExportTiffDialog::dragEnterEvent(QDragEnterEvent *event)
{
    if (event->mimeData()->hasUrls()) {
        ...
        if (file.exists()
            && file.suffix().indexOf(QRegularExpression("tif"), QRegularExpression::CaseInsensitiveOption) != -1)
            event->acceptProposedAction();
    }
}
...
void DkExportTiffDialog::createLayout()
{
    ...
    connect(mFileEdit, &QLineEdit::textChanged, this, &DkExportTiffDialog::onFileEditTextChanged);
...
void DkExportTiffDialog::onOpenButtonPressed()
{
    // load system default open dialog
    QString fileName = QFileDialog::getOpenFileName(this,
                                                    tr("Open TIFF"),
                                                    mFilePath,
                                                    DkSettingsManager::param()
                                                        .app()
                                                        .saveFilters.filter(QRegularExpression(".*tif.*"))
                                                        .join(";;"),
                                                    nullptr,
                                                    DkDialog::fileDialogOptions());
...
void DkExportTiffDialog::onSaveButtonPressed()
{
...
void DkExportTiffDialog::onFileEditTextChanged(const QString &filename)
{
...
void DkExportTiffDialog::accept()
{
    ...
    QFuture<int> future = QtConcurrent::run([&, suffix] {
        ...
        return nmc::DkExportTiffDialog::exportImages(sFile.absoluteFilePath(),
                                                     mFromPage->value(),
                                                     mToPage->value(),
...
int DkExportTiffDialog::exportImages(const QString &saveFilePath, int from, int to, bool overwrite)
{
    ...
    for (int idx = from; idx <= to; idx++) {
        ...
        QString lSaveFilePath = mLoader.save(cInfo.absoluteFilePath(),
                                             mLoader.image(),
                                             90); // TODO: ask user for compression?
...
void DkMosaicDialog::createLayout()
{
    ...
    connect(mDarkenSlider, &QSlider::valueChanged, this, &DkMosaicDialog::onDarkenSliderValueChanged);

    mLightenSlider = new QSlider(Qt::Horizontal, this);
    mLightenSlider->setValue(40);
    connect(mLightenSlider, &QSlider::valueChanged, this, &DkMosaicDialog::onLightenSliderValueChanged);
    ...
    connect(mNewWidthBox,
            QOverload<int>::of(&QSpinBox::valueChanged),
            this,
            &DkMosaicDialog::onNewWidthBoxValueChanged);
    ...
    connect(mNewHeightBox,
            QOverload<int>::of(&QSpinBox::valueChanged),
            this,
            &DkMosaicDialog::onNewHeightBoxValueChanged);
    ...
    connect(mNumPatchesH,
            QOverload<int>::of(&QSpinBox::valueChanged),
            this,
            &DkMosaicDialog::onNumPatchesHValueChanged);
    ...
    connect(mNumPatchesV,
            QOverload<int>::of(&QSpinBox::valueChanged),
            this,
            &DkMosaicDialog::onNumPatchesVValueChanged);
    ...
    connect(mFilterEdit, &QLineEdit::textChanged, this, &DkMosaicDialog::onFileEditTextChanged);
    ...
    // mButtons
    mButtons = new QDialogButtonBox(QDialogButtonBox::Apply | QDialogButtonBox::Save | QDialogButtonBox::Cancel,
                                    Qt::Horizontal,
                                    this);
...
void DkMosaicDialog::onOpenButtonPressed()
{
    // load system default open dialog
    QString fileName = QFileDialog::getOpenFileName(this,
                                                    tr("Open TIFF"),
...
void DkMosaicDialog::onDbButtonPressed()
{
    qDebug() << "save triggered...";
...
void DkMosaicDialog::onFileEditTextChanged(const QString &filename)
{
    qDebug() << "new file name: " << filename;
}

void DkMosaicDialog::onNewWidthBoxValueChanged(int)
{
    if (!mLoader.hasImage())
        return;
    ...
    mRealResLabel->setText(tr("%1 x %2 cm @150 dpi")
                               .arg(mNewWidthBox->value() / 150.0 * 2.54, 0, 'f', 1)
                               .arg(mNewHeightBox->value() / 150.0 * 2.54, 0, 'f', 1));
    updatePatchRes();
}

void DkMosaicDialog::onNewHeightBoxValueChanged(int)
{
    if (!mLoader.hasImage())
        return;
...
void DkMosaicDialog::onNumPatchesHValueChanged(int)
{
    if (!mLoader.hasImage())
        return;

    mNumPatchesV->blockSignals(true);
    mNumPatchesV->setValue(
        qFloor((float)mLoader.image().height() / ((float)mLoader.image().width() / mNumPatchesH->value())));
    mNumPatchesV->blockSignals(false);
    updatePatchRes();
}

void DkMosaicDialog::onNumPatchesVValueChanged(int)
{
    if (!mLoader.hasImage())
        return;

    mNumPatchesH->blockSignals(true);
    mNumPatchesH->setValue(
        qFloor((float)mLoader.image().width() / ((float)mLoader.image().height() / mNumPatchesV->value())));
    mNumPatchesH->blockSignals(false);
    updatePatchRes();
}

void DkMosaicDialog::onDarkenSliderValueChanged(int)
{
    updatePostProcess();
}

void DkMosaicDialog::onLightenSliderValueChanged(int)
{
    updatePostProcess();
}

void DkMosaicDialog::onSaturationSliderValueChanged(int)
{
    updatePostProcess();
}
    ...
    if (button == mButtons->button(QDialogButtonBox::Save)) {
        ...
        if (!mMosaic.isNull()) {
            ...
            QFuture<bool> future = QtConcurrent::run([&] {
                return postProcessMosaic(mDarkenSlider->value() / 100.0f,
...
int DkMosaicDialog::computeMosaic(const QString &filter, const QString &suffix, int newWidth, int numPatchesH)
{
    ...
    cc.setTo(0);
    cv::Mat ccD(numPatches.height(),
                numPatches.width(),
                CV_8UC1); // tells us if we have already computed the real patch
    ...
    while (iDidNothing < 10000) {
        ...
        try {
            ...
            if (maxVal > ccPtr[maxIdx.x]) {
                ...
                mFilesUsed[maxIdx.y * numPatchesH + maxIdx.x] = QFileInfo(
                    imgPath); // replaces additionally the old file
                iDidNothing = 0;
            } else
                iDidNothing++;
        }
    ...
    for (int rIdx = 0; rIdx < ccD.rows; rIdx++) {
        ...
        for (int cIdx = 0; cIdx < ccD.cols; cIdx++) {
            ...
            if (!cFile.exists()) {
                emit infoMessage(
                    tr("Something is seriously wrong, I could not load: %1").arg(cFile.absoluteFilePath()));
                continue;
            }
...
bool DkMosaicDialog::postProcessMosaic(float multiply /* = 0.3 */,
                                       float screen /* = 0.5 */,
                                       float saturation,
                                       bool computePreview)
{
...
void DkWelcomeDialog::accept()
{
    ...
    // change language
    if (mLanguageCombo->currentIndex() != mLanguages.indexOf(DkSettingsManager::param().global().language)
        && mLanguageCombo->currentIndex() >= 0) {
...
void DkArchiveExtractionDialog::openArchive()
{
    ...
    QString filePath = QFileDialog::getOpenFileName(this,
                                                    tr("Open Archive"),
                                                    (mArchivePathEdit->text().isEmpty())
                                                        ? QFileInfo(mFilePath).absolutePath()
                                                        : mArchivePathEdit->text(),
                                                    tr("Archives (%1)")
                                                        .arg(
                                                            DkSettingsManager::param().app().containerRawFilters.remove(
                                                                ",")),
                                                    nullptr,
                                                    DkDialog::fileDialogOptions());
...
{
    QStringList extractedFiles = extractFilesWithProgress(mArchivePathEdit->text(),
                                                          mFileList,
                                                          mDirPathEdit->text(),
                                                          mRemoveSubfolders->isChecked());

    if ((extractedFiles.isEmpty() || extractedFiles.size() != mFileList.size())
        && !extractedFiles.contains("userCanceled")) {
...
QStringList DkArchiveExtractionDialog::extractFilesWithProgress(const QString &fileCompressed,
...
void DkDialogManager::openAppManager() const
{
    ...
    connect(appManagerDialog,
            ...
            am.appManager(),
            &DkAppManager::openFileSignal); // forward
...
void DkSvgSizeDialog::createLayout()
{
    ...
    connect(mSizeBox[b_height],
            QOverload<int>::of(&QSpinBox::valueChanged),
            this,
            &DkSvgSizeDialog::onHeightValueChanged);
...
void DkSvgSizeDialog::onWidthValueChanged(int val)
{
    mSize.setWidth(val);
...
void DkSvgSizeDialog::onHeightValueChanged(int val)
{
